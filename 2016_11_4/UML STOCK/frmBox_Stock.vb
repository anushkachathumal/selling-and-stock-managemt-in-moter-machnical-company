...
Public Class frmBox_Stock

    Dim c_dataCustomer1 As DataTable
    Dim c_dataCustomer2 As DataTable
    Dim _Itemcode As String

    Private Sub frmBox_Stock_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        txtDate.Text = Today
    ...
    Function Load_Date()
        ...
        Try
            ...
            Sql = "SELECT * FROM M07Product_Box WHERE M07Status='A' "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            For Each DTRow1 As DataRow In M01.Tables(0).Rows
                _QTY = 0
                'OPERNING BALANCE
                Sql = "SELECT SUM(S03Qty) AS QTY FROM S03Box_Stock WHERE S03Status in ('A','HD') AND S03Tr_Type='OB' AND S03Box_No='" & Trim(M01.Tables(0).Rows(i)("M07Ref")) & "'  GROUP BY S03Box_No"
                M02 = DBEngin.ExecuteDataset(con, Nothing, Sql)
                If isValidDataset(M02) Then
                    _QTY = M02.Tables(0).Rows(0)("QTY")
                End If
                'STOCK IN
                Sql = "SELECT SUM(S03Qty) AS QTY FROM S03Box_Stock WHERE S03Status in ('A','HD') AND S03Tr_Type='SI' AND S03Box_No='" & Trim(M01.Tables(0).Rows(i)("M07Ref")) & "'  GROUP BY S03Box_No"
                M02 = DBEngin.ExecuteDataset(con, Nothing, Sql)
                If isValidDataset(M02) Then
                ...
                'PACKING
                Sql = "SELECT SUM(S03Qty) AS QTY FROM S03Box_Stock WHERE S03Status in ('A','HD') AND S03Tr_Type='PK' AND S03Box_No='" & Trim(M01.Tables(0).Rows(i)("M07Ref")) & "'  GROUP BY S03Box_No"
                M02 = DBEngin.ExecuteDataset(con, Nothing, Sql)
                If isValidDataset(M02) Then
                    _QTY = _QTY - M02.Tables(0).Rows(0)("QTY")
                End If
                'Transfer
                Sql = "SELECT SUM(S03Qty) AS QTY FROM S03Box_Stock WHERE S03Status in ('A','HD') AND S03Tr_Type='TR' AND S03Box_No='" & Trim(M01.Tables(0).Rows(i)("M07Ref")) & "'  GROUP BY S03Box_No"
                M02 = DBEngin.ExecuteDataset(con, Nothing, Sql)
                If isValidDataset(M02) Then
                    _QTY = _QTY - M02.Tables(0).Rows(0)("QTY")
                End If
    ...
    Function Load_Combo()
        ...
        Try
            Sql = "select M07Name as [##] from M07Product_Box where M07Status='A' order by M07Ref "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            With cboItem
                .DataSource = M01
    ...
    Function Search_Itemcode() As Boolean
        ...
        Try
            Sql = "select * from M07Product_Box where M07Status='A' and M07Name='" & cboItem.Text & "' "
            M01 = DBEngin.ExecuteDataset(con, Nothing, Sql)
            If isValidDataset(M01) Then
                ...
                Sql = "SELECT SUM(S03Qty) AS QTY FROM S03Box_Stock WHERE S03Status in ('A','HD') AND S03Tr_Type='OB' AND S03Box_No='" & Trim(M01.Tables(0).Rows(0)("M07Ref")) & "' GROUP BY S03Box_No"
                M02 = DBEngin.ExecuteDataset(con, Nothing, Sql)
                If isValidDataset(M02) Then
                    _qty = M02.Tables(0).Rows(0)("QTY")
                ...
                Sql = "SELECT SUM(S03Qty) AS QTY FROM S03Box_Stock WHERE S03Status in ('A','HD') AND S03Tr_Type='SI' AND S03Box_No='" & Trim(M01.Tables(0).Rows(0)("M07Ref")) & "'  GROUP BY S03Box_No"
                M02 = DBEngin.ExecuteDataset(con, Nothing, Sql)
                If isValidDataset(M02) Then
                    _qty = _qty + M02.Tables(0).Rows(0)("QTY")
                ...
                _stockIn = 0
                'SALES
                Sql = "SELECT SUM(S03Qty) AS QTY FROM S03Box_Stock WHERE S03Status in ('A','HD') AND S03Tr_Type='PK' AND  S03Box_No='" & Trim(M01.Tables(0).Rows(0)("M07Ref")) & "'  GROUP BY S03Box_No"
                M02 = DBEngin.ExecuteDataset(con, Nothing, Sql)
                If isValidDataset(M02) Then
                    _qty = _qty + M02.Tables(0).Rows(0)("QTY")
                    _stockIn = M02.Tables(0).Rows(0)("QTY")
                End If
                Sql = "SELECT SUM(S03Qty) AS QTY FROM S03Box_Stock WHERE S03Status in ('A','HD') AND S03Tr_Type='TR' AND  S03Box_No='" & Trim(M01.Tables(0).Rows(0)("M07Ref")) & "'  GROUP BY S03Box_No"
                M02 = DBEngin.ExecuteDataset(con, Nothing, Sql)
                If isValidDataset(M02) Then
                    _qty = _qty + M02.Tables(0).Rows(0)("QTY")
